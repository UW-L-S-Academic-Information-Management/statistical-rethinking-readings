...
curve(dbeta2(x,mean(logistic(post$a[,gid])), mean(post$theta)), from = 0,
      to = 1, ylab = "Density", xlab = "probability admit", ylim=c(0,3),
      lwd=2)
...
m12.4q <- quap(
  ...
  ), data = d, start = list(a1 = -2, a2 = -1, a3 = 0, a4 = 1,
                            a5 = 2, a6 = 2.5)
)
...
plot(NULL, type = "n", xlab = "intention", ylab = "probability", xlim = c(0,1),
...
for(s in 1:50){
  ...
  for (i in 1:6) lines(kI, pk[,i], col=gray(.1))
}
...
m12.7 <- ulam(
  alist(
    R ~ ordered-logistic(mu, cutpoints), 
